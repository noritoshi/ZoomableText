 ...
 */
public class InteractiveLineGraphView extends FrameLayout {
	...
	private ScaleableView mScaleableView;
	...
	protected void onFinishInflate() {
		...
		mScaleableView = (ScaleableView) findViewById(R.id.scaleable_view);
	}
...
//	@SuppressLint("NewApi")
	...
	private final ScaleGestureDetector.OnScaleGestureListener mScaleGestureListener = new ScaleGestureDetector.SimpleOnScaleGestureListener() {
		...
		public boolean onScale(ScaleGestureDetector scaleGestureDetector) {
			...
			float scale = (float) Math.sqrt((lastSpanX * lastSpanX + lastSpanY * lastSpanY)
					/ (spanX * spanX + spanY * spanY));

			float newWidth = scale * mCurrentViewport.width();
			float newHeight = scale * mCurrentViewport.height();
	...
	private void scaleViews() {
		float scale = 2 / mCurrentViewport.width();
		mScaleableView.onScale(scale);
	}

	private void scrollViews() {
		mScaleableView.scroll((mCurrentViewport.left - AXIS_X_MIN) * mContentRect.width() / 2,
				(-mCurrentViewport.bottom + AXIS_Y_MAX) * mContentRect.height() / 2);
	}
	...
	private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
		...
		public boolean onDown(MotionEvent e) {
			InteractiveLineGraphView.super.dispatchTouchEvent(e);
...
//			scaleChildren();
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			...
			requestLayout();
//
//			Log.d("nori", "VP x:" + mCurrentViewport.left + ",y:" + mCurrentViewport.top);
//			mScaleableView.scroll((mCurrentViewport.left - AXIS_X_MIN) * mContentRect.width() / 2,
//					(-mCurrentViewport.bottom + AXIS_Y_MAX) * mContentRect.height() / 2);
//			Log.d("nori", "vp.bottom:" + mCurrentViewport.bottom);
			return true;
		}
		...
		public boolean onSingleTapUp(MotionEvent e) {
			return InteractiveLineGraphView.super.dispatchTouchEvent(e);
		}
	};
...
//				(-mCurrentViewport.bottom + AXIS_Y_MAX) * mContentRect.height() / 2);
		...
		ViewCompat.postInvalidateOnAnimation(this);
//		ViewCompat.postInvalidateOnAnimation(mScaleableView);
	}
...
//			ViewCompat.postInvalidateOnAnimation(mScaleableView);
	...
	public void setEdgeEffect(EdgeEffectView edgeEffectView) {
		mEdgeEffectView = edgeEffectView;
	}
